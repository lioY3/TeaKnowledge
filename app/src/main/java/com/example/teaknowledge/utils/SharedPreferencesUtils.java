...
public class SharedPreferencesUtils {
    ...
    public static <T> List<T> getListData1(String key) {
        List<T> list = new ArrayList<>();
        String json = sp.getString(key, "");
        if (!json.equals("") && json.length() > 0) {
            Gson gson = new Gson();
            JsonArray array = new JsonParser().parse(json).getAsJsonArray();
            for (JsonElement elem : array) {
                if(gson.fromJson(elem, TeaInfo.class).type == 0)
                    list.add(gson.fromJson(elem, (Type) News.class));
                else if(gson.fromJson(elem, TeaInfo.class).type == 1)
                    list.add(gson.fromJson(elem, (Type) Goods.class));
            }
        }
        return list;
    }
